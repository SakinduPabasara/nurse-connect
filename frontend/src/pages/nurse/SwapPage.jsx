...
export default function SwapPage() {
  ...
  const fetchSwaps = useCallback(async (silent = false) => {
    ...
    try {
      const { data } = await API.get("/swap/my");
      setSwaps(data);
    } catch {
      setSwaps([]);
    } finally {
  ...
  const fetchMyRoster = useCallback(async () => {
    try {
      ...
      setMyRoster(
        Array.isArray(data) ? data.filter((s) => s.date >= today) : [],
      );
    } catch {
      setMyRoster([]);
    }
  ...
  useEffect(() => {
    ...
    const match = myRoster.find(
      (s) => s.date === requesterShiftDate && s.shift === requesterShift,
    );
    if (match) {
  ...
  useEffect(() => {
    ...
    return () => {
      if (socket) socket.off("swap:updated");
    };
  }, [fetchSwaps]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.targetNurse || !form.myShiftId || !form.targetShiftId) {
      setMsg({
        type: "error",
        text: "Please select your shift and the target nurse shift.",
      });
      return;
    }
    ...
    if (!myEntry || !targetEntry) {
      setMsg({
        type: "error",
        text: "Invalid shift selection. Please try again.",
      });
      return;
    }
    setSubmitting(true);
  ...
  const handleRespond = async (id, status) => {
    try {
      ...
      fetchSwaps();
      notify.success(`Swap request ${status}.`);
    } catch (err) {
      ...
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">
            Facilitate nurse-to-nurse shift exchanges with full tracking
          </div>
        </div>
        ...
        >
          <button
            className={`chip-tab ${tab === "list" ? "active" : ""}`}
            onClick={() => setTab("list")}
          >
            Active Requests
            {pending > 0 && (
          ...
          <button
            className={`chip-tab ${tab === "new" ? "active" : ""}`}
            onClick={() => setTab("new")}
          >
            + New Request
          </button>
        ...
        loading ? (
          ...
          <div className="empty-state">
            ...
            <div className="empty-state-text">
              No shift swap activity recorded
            </div>
            <div className="empty-state-sub">
              Initiate a swap request with a nurse
            </div>
            <button
                    ...
                    {s.status === "pending" && isTargetNurse && (
                      <div style={{ display: "flex", gap: 8 }}>
                        <button
                          ...
                          onClick={() => handleRespond(s._id, "approved")}
                        >
                          ✓ Accept
                        </button>
                        <button
        ...
        >
          <div style={{ marginBottom: 26 }}>
            ...
            <div style={{ fontSize: "0.78rem", color: "var(--text3)" }}>
              You can only swap shifts that are actually assigned to you by the
              admin
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: "grid", gap: 20 }}>
            ...
            >
              ...
              <div>
                ...
                {form.targetShiftId &&
                  (() => {
                    const e = targetRoster.find(
                      (s) => s._id === form.targetShiftId,
                    );
                    return e ? (
                      <div
                        style={{
            ...
            <button
              ...
              type="submit"
              disabled={submitting || !form.myShiftId || !form.targetShiftId}
            >
              {submitting ? (
                <>
                  <span
