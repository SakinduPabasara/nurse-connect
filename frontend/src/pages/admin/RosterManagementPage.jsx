...
const SHIFT_MAP = {
  "7AM-1PM": { label: "M", title: "Morning", color: "#60a5fa", bg: "rgba(37,99,235,0.1)", border: "rgba(37,99,235,0.2)" },
  "1PM-7PM": { label: "A", title: "Afternoon", color: "#22d3ee", bg: "rgba(6,182,212,0.1)", border: "rgba(6,182,212,0.2)" },
  "7AM-7PM": { label: "F", title: "Full Day", color: "#fbbf24", bg: "rgba(245,158,11,0.1)", border: "rgba(245,158,11,0.2)" },
  "7PM-7AM": { label: "N", title: "Night", color: "#a78bfa", bg: "rgba(139,92,246,0.1)", border: "rgba(139,92,246,0.2)" },
};
...
export default function RosterManagementPage() {
  ...
  const [form, setForm] = useState({ nurse: "", ward: "", dates: [""], shift: "7AM-1PM", month: "" });
  ...
  useEffect(() => {
    API.get("/auth/users?role=nurse&isVerified=true").then(r => setNurses(r.data)).catch(() => {});
  ...
  const fetchRoster = async () => {
    setLoading(true);
    try {
      const { data } = await API.get(`/roster/all?month=${month}&ward=${rosterWard}`);
      setRoster(data);
    } catch { setRoster([]); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetchRoster(); }, [rosterWard, month]);

  const handleCreate = async (e) => {
    e.preventDefault();
    const cleanedDays = [...new Set(form.dates.filter(Boolean))];
    if (!form.nurse || !form.ward || !form.month || cleanedDays.length === 0) {
       notify.error("Incomplete deployment configuration."); return;
    }
    setSubmitting(true);
    try {
      const dates = cleanedDays.map(d => `${form.month}-${String(d).padStart(2, "0")}`);
      await API.post("/roster/bulk", { ...form, dates });
      notify.success("Roster session broadcasted.");
      setForm({ nurse: "", ward: "", dates: [""], shift: "7AM-1PM", month: "" });
      fetchRoster(); setTab("view");
    } catch (err) { notify.error("Deployment failure."); }
    finally { setSubmitting(false); }
  };

  const handleDelete = async (id) => {
    const isConfirmed = await confirm({ title: "Cancel Shift", message: "Confirm cancellation of this roster entry?", confirmText: "Cancel Shift" });
    if (!isConfirmed) return;
    try {
      await API.delete(`/roster/${id}`);
      fetchRoster(); notify.success("Entry voided.");
    } catch (err) { notify.error("Action denied."); }
  };
  ...
  return (
    <div className="roster-mgmt-container" style={{ animation: 'fadeIn 0.4s ease' }}>
      ...
      {tab === 'create' ? (
        <div className="form-card-premium" style={{ maxWidth: 800, margin: '40px auto' }}>
           <h3 style={{ fontSize: '1.2rem', fontWeight: 800, marginBottom: 24 }}>Strategic Shift Allocation</h3>
           <form onSubmit={handleCreate} style={{ display: 'grid', gap: 24 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                 <div className="form-group">
                    ...
                    <SearchableSelect options={hospitals.map(h => ({ value: h.name, label: h.name }))} value={selectedHospital} onChange={setSelectedHospital} placeholder="Filter by Institution" />
                 </div>
                 <div className="form-group">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
                 ...
                 <div className="form-group">
                    ...
                    <select className="form-input" value={form.shift} onChange={e => setForm({...form, shift: e.target.value})} style={{ background: 'var(--bg2)', height: 48 }}>
                       {SHIFTS.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                 </div>
              </div>

              <div style={{ background: 'var(--bg2)', padding: 24, borderRadius: 20, border: '1px dashed var(--border)' }}>
                 <label className="form-label" style={{ marginBottom: 16, display: 'block' }}>Duty Sequence (Select Days)</label>
                 <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                    {dayOptions.map(d => (
                       <button key={d} type="button" className={`btn btn-sm ${form.dates.includes(d) ? 'btn-primary' : 'btn-ghost'}`} style={{ borderRadius: 8, width: 44, height: 44 }} onClick={() => {
                          setForm(f => ({ ...f, dates: f.dates.includes(d) ? f.dates.filter(x => x !== d) : [...f.dates, d] }));
                       }}>{d}</button>
                    ))}
                 </div>
          ...
          <div className="filter-bar-premium" style={{ background: 'var(--surface)', padding: 16, borderRadius: 18, border: '1px solid var(--border)', display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap', marginTop: 24 }}>
             ...
             <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
                {Object.entries(SHIFT_MAP).map(([s, cfg]) => (
                   <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.75rem', fontWeight: 600 }}>
                      <div style={{ width: 22, height: 22, borderRadius: 6, background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem' }}>{cfg.label}</div>
          ...
          <div className="roster-explorer">
             {loading ? (
                ...
                <div className="empty-state" style={{ padding: 100 }}>
                   ...
                   <div className="empty-state-text">No workforce sequence recorded for this cycle.</div>
                </div>
             ) : (
                <div style={{ overflowX: 'auto' }}>
                   <table className="roster-table">
                      ...
                      <tbody>
                         {Object.entries(groupedRoster).sort().map(([name, days]) => (
                            <tr key={name}>
                               ...
                               {daysInMonth.map(d => {
                                  ...
                                  return (
                                     <td key={d}>
                                        {entry && cfg && (
                                           <div className="duty-marker" style={{ background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.border}` }} onClick={() => handleDelete(entry._id)}>
                                              {cfg.label}
