...
const createSwap = async (req, res) => {
  ...
  if (
    ...
    return res
      ...
      .json({
        message:
          "Please provide targetNurse, both shift dates, and both shifts",
      });
  }
  ...
  if (targetNurse === req.user._id.toString()) {
    return res
      .status(400)
      .json({ message: "You cannot send a swap request to yourself" });
  }
  if (
    !VALID_SHIFTS.includes(requesterShift) ||
    !VALID_SHIFTS.includes(targetShift)
  ) {
    return res
      .status(400)
      .json({ message: `Shifts must be one of: ${VALID_SHIFTS.join(", ")}` });
  }
  // ------------------
  ...
  try {
    ...
    if (target.ward !== req.user.ward) {
      return res
        .status(400)
        .json({
          message: "You can only request a swap with a nurse in your same ward",
        });
    }
    ...
    if (!myRosterEntry) {
      return res
        .status(400)
        .json({
          message: `You do not have a "${requesterShift}" shift assigned on ${requesterShiftDate}. You can only request a swap for your actual assigned shifts.`,
        });
    }
    ...
    if (!targetRosterEntry) {
      return res
        .status(400)
        .json({
          message: `The target nurse does not have a "${targetShift}" shift assigned on ${targetShiftDate}.`,
        });
    }
...
const respondToSwap = async (req, res) => {
  ...
  if (!status) {
    return res
      .status(400)
      .json({ message: "Please provide a status (approved or rejected)" });
  }
  ...
  try {
    ...
    if (swap.targetNurse._id.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only the target nurse can respond to this request" });
    }
    ...
    if (swap.status !== "pending") {
      return res
        .status(400)
        .json({ message: `This swap request is already ${swap.status}` });
    }
    ...
    if (status === "approved") {
      ...
      if (requesterRosterEntry && targetRosterEntry) {
        ...
        rosterWarning =
          "Swap approved but no matching roster entries were found for the specified dates. Roster was not automatically updated.";
    ...
    try {
      getIO()
        .to("user:" + swap.requester._id.toString())
        .emit("notification:new", notif);
      getIO()
        .to("user:" + swap.requester._id.toString())
        .emit("swap:updated");
      if (status === "approved") {
        ...
        getIO()
          ...
          .emit("roster:updated");
        getIO()
          .to("user:" + swap.targetNurse._id.toString())
          .emit("roster:updated");
      }
    } catch (err) {
      console.error("Socket emit error:", err.message);
...
// @DELETE /api/swap/:id  — Admin deletes a swap request
const deleteSwap = async (req, res) => {
  ...
  try {
    ...
    const notifyUser = async (userId, name) => {
      if (!userId) return null;
      return Notification.create({
        recipient: userId,
        message: `Your shift swap request${name ? ` involving ${name}` : ""} was removed by admin.`,
        type: "swap",
      });
    };

    const [notifRequester, notifTarget] = await Promise.all([
      notifyUser(swap.requester?._id, swap.targetNurse?.name),
      notifyUser(swap.targetNurse?._id, swap.requester?.name),
    ]);

    try {
      if (swap.requester?._id) {
        if (notifRequester) {
          getIO()
            .to("user:" + swap.requester._id.toString())
            .emit("notification:new", notifRequester);
        }
        getIO().to("user:" + swap.requester._id.toString()).emit("swap:updated");
      }
      if (swap.targetNurse?._id) {
        if (notifTarget) {
          getIO()
            .to("user:" + swap.targetNurse._id.toString())
            .emit("notification:new", notifTarget);
        }
        getIO().to("user:" + swap.targetNurse._id.toString()).emit("swap:updated");
      }
      getIO().to("admin").emit("swap:updated");
    } catch (err) {
      console.error("Socket emit error:", err.message);
    }
...
module.exports = { createSwap, getMySwaps, respondToSwap, getAllSwaps, deleteSwap };
